...
class ReviewReportController {
    ...
    static async addReviewReport(req, res) {
        try {
            const { review_id, reason, status } = req.body;
            const account_id = req.body.account_id; // hoặc lấy từ token đã decode

            // Kiểm tra rỗng
            if (!review_id || !account_id || !reason || !status) {
                return res.status(400).json({
                    success: false,
                    message: "Vui lòng nhập đầy đủ các trường review_id, account_id, reason và status."
                });
            }

            const report = await ReviewReportService.addReviewReport(review_id, account_id, reason, status);
