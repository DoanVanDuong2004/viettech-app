...
async function updateOldReviews() {
    try {
        const result = await reviewModel.updateMany(
            { status: { $exists: false } }, // Những review chưa có trường status
            { $set: { status: 'active' } }
        );
        console.log(`Đã cập nhật ${result.modifiedCount} review.`);
        process.exit();
    } catch (error) {
...
class ReviewController {
    ...
    static async addReview(req, res) {
        try {
            const { account_id, product_id, contents_review, rating, image_ids, bill_id } = req.body;
            ...
            const review = await reviewService.addReview(account_id, product_id, contents_review, rating, image_ids, bill_id);
    ...
    static async getReviewsByAccount(req, res) {
        try {
            const { accountId } = req.params;
            const reviews = await reviewService.getReviewsByAccountId(accountId);
            res.status(200).json({ success: true, data: reviews });
        } catch (error) {
            res.status(500).json({ success: false, message: error.message });
        }
    ...
    static async updateReview(req, res) {
        try {
            ...
            const { contents_review, rating, image_ids } = req.body;
    ...
    static async getReviewManagementPage(req, res) {
        try {
            ...
            let reviewSortOption = { createdAt: -1 }; // mặc định là mới nhất
            ...
            if (reportStatus === "Đã xử lý") {
                reportFilter.status = "reported";
            } else if (reportStatus === "Đang chờ") {
                reportFilter.status = { $ne: "reported" }; // chưa duyệt
            }
    ...
    static async updateReportStatus(req, res) {
        try {
            ...
            const userId = updatedReport.account_id?._id?.toString();
            if (userId) {
                let title = '', message = '';

                if (status === 'reported') {
                    title = "Báo cáo của bạn đã được duyệt";
                    message = "Cảm ơn bạn đã đóng góp! Báo cáo của bạn đã được xử lý thành công.";
                } else {
                    title = "Báo cáo của bạn không được duyệt";
                    message = "Rất tiếc, báo cáo bạn gửi chưa đủ điều kiện để chấp nhận.";
                }
