...
class ReviewService {
    static async addReview(account_id, product_id, contents_review, rating, image_ids = [], bill_id) {
        try {
            // Kiểm tra đầu vào
            if (!account_id || !product_id || !contents_review || typeof rating !== "number" || !bill_id) {
                return {
                    success: false,
                    message: "Thiếu thông tin: account_id, product_id, contents_review, rating (number) và bill_id là bắt buộc."
                };
            }
            ...
            if (rating < 1 || rating > 5) {
                return {
                    success: false,
                    message: "Rating phải là một số từ 1 đến 5."
                };
            }
    
            // Tìm xem review đã tồn tại chưa
            let existingReview = await Review.findOne({ account_id, product_id, bill_id });
    
            const now = new Date();
            if (existingReview) {
                return {
                    success: false,
                    message: "Review cho sản phẩm này đã tồn tại. Vui lòng cập nhật review hiện có."
                };
            }
            ...
            return {
                success: true,
                message: "Review thêm thành công",
                data: {
                    ...
                    bill_id: existingReview.bill_id,
        ...
        try {
            ...
            const populatedReviews = await Promise.all(reviews.map(async (review) => {
                ...
                return {
                    ...
                    bill_id: review.bill_id, // Thêm trường bill_id vào review
    ...
    static async getReviewsByAccountId(accountId) {
        try {
            // Lấy danh sách review của tài khoản theo account_id
            const reviews = await Review.find({ account_id: accountId }).sort({ createdAt: -1 });
            ...
            const populatedReviews = await Promise.all(reviews.map(async (review) => {
                ...
                return {
                    ...
                    bill_id: review.bill_id, // Trả về bill_id nếu có
                    createdAt: review.createdAt,
                    updatedAt: review.updatedAt,
                    images: images.map(image => ({
                        _id: image._id,
            ...
            throw new Error("Lỗi khi lấy danh sách review của tài khoản: " + error.message);
        }
    }
}
